...
void replay_aof() {
  ...
  if (last_rewrite_size > REWRITE_THRESHOLD) {
    rewrite_aof();
    last_rewrite_size = fs::file_size(p);
  }
}
...
void rewrite_aof() {
  ...
  if (!temp_aof.is_open()) {
    std::cerr << "[AOF Error] Could not open temporary file for rewrite."
              << std::endl;
    return;
  }
  ...
  try {
    if (aof_file.is_open())
      aof_file.close(); // close main aof file
    std::filesystem::rename(temp_path,
                            main_path); // remain temp file to main aof file
    ...
    std::cout << "[AOF] Rewrite complete. File compressed successfully."
              << std::endl;
...
void check_and_rewrite_aof() {
  ...
  try {
    ...
    if (fs::exists(p)) {
      ...
      if (size > REWRITE_THRESHOLD && size > last_rewrite_size * 2) {
        rewrite_aof();
      }
    }
