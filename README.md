...
- **Connection Management:** Maintains an active socket watchlist with $O(1)$ connection teardown natively via the Linux `epoll` kernel structure, minimizing latency during client disconnects.
...
- **Any OS** (macOS, Windows, FreeBSD, etc., via Docker Compose)
...
For local Linux build, follow the steps:
